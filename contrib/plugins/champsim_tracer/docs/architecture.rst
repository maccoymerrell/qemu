...
1. **TCG translation** (once per TB, on first sight).  The tracer
   reads the facts the target's own decode rules stated — the generic
   word, the register sets, the per-access provenance — through
   ``qemu-plugin-dataflow.h``, and joins the word against the
   vocabulary table.  Cost is paid once per static basic block, so it amortises
...
* **Slowdown vs bare QEMU:** tracing without WP costs ~11× on this
  ...
  vocabulary join + chain-assembly walk.  Enabling WP
...
.. list-table::
   ...
   * - ``champsim_tracer_vocabulary.{h,cc}``
     - The words QEMU states and what each one means on the wire: one
       sorted array joining a generic word a decode rule may carry
       (``int.add``, ``fp.mul``, ...) to the ``GenericOpcode`` and
       ``BranchType`` the wire publishes for it.  It is one table, not
       four; it infers nothing from operands or register classes,
       because the word already carries the domain; and it refuses a
       word this build cannot read, since that means the emulator and
       the plugin disagree about the vocabulary.
   * - ``champsim_tracer_qdep.{h,cc}``
     - The wire's dataflow facts, read from QEMU through
       ``qemu-plugin-dataflow.h``: the per-instruction source and
       destination register sets, the per-access address and data
       provenance, and the fold notes the target's own decode sites
       state.  Nothing here consults a second decoder.
   * - ``champsim_tracer_mnemonics.h``
     - The shared per-instruction field surface: ``InsnFields`` (the
       decoded generic fields a template carries), the
       template-static operand caps that fix the dep-mask bit layout,
       ``QemuRegKey`` (the feature/name pair a register value is read
       through), and the per-ISA architectural-property function
       pointers (delay slots, marker encodings, address
       canonicalisation).
   * - ``champsim_tracer_mnemonic_audit.py``
     - Offline.  Enumerates the generic vocabulary the plugin
       publishes so the external referee
       (``tools/cst_referee.py``) can be scored against the same
       word set.  It is not part of the build.
...
A system register is not a second numbering space here.  It reaches the
plugin the same way every other register does: the target's decode rule
states the access, the emulator's register file declares the storage,
and the generated ``champsim_tracer_regmap_<isa>.h`` /
``champsim_tracer_gdbmap_<isa>.h`` tables join what the target
registers to the generic ID the wire publishes.  What makes the class
worth its own section is that the join has to be built from sources the
targets keep in different places — AArch64's
``insn_dataflow_declare_regfile()`` rows, RISC-V's ``csr_ops`` table
(371 CSRs), MIPS's coprocessor registers — and that the generator
refuses in both directions, so a target that grows a control register
without an adjudicated row fails the build rather than shipping a map
one register short.

The direction is the decode rule's, not a decoder's guess.  ``MRS``
reads and ``MSR`` writes because the AArch64 rule says so at the site
that emits the access, and Zicsr follows its architectural suppression
rules — ``csrrw`` with ``rd == x0`` does not read, ``csrrs``/``csrrc``
with a zero ``rs1`` does not write — because the RISC-V rule states
each half separately.  Getting that wrong is not cosmetic: publishing
``csrr a0, vl`` as a write of ``vl`` would reroute every following
vector instruction's configuration edge onto it.

One consequence of the generic-ID layer is worth stating:
...
An ISA whose control registers the target already exposes as ordinary
registers needs nothing extra: MIPS declares its coprocessor registers
in the same regfile the general registers live in, so they join through
``champsim_tracer_regmap_mipsel.h`` like any other register.

One consequence reaches the tooling.  A system register's dependency is
recorded at the granularity of its generic ID, so the external referee
(``tools/cst_referee.py``) and the scorers that join its output compare
AGAINST THAT CLASS, not against the architectural name: ``mrs x0,
nzcv`` reads ``REG_FLAGS``, and the fact that it was spelled ``NZCV``
rather than ``PSTATE`` is not a dependency fact.  A register newly
given a class of its own needs its name added to the referee's
name-to-class correspondence, or the comparison will read a genuine
...
* **Undecoded instruction** — ``qemu_plugin_insn_undecoded`` reports
  that no rule in the target's own decoder matched these bytes.  Real
  code does not fail to decode.  The signal is the emulator's, not a
  second decoder's: what the CPU refuses to execute is exactly what
  the tracer refuses to record.
