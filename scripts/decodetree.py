...
def ident_table_name():
    return decode_function + '_ident_tab'


def ident_qual_name(pat):
    """The identity a consumer keys on.

       Qualified by the decode FUNCTION because a target has several
...
def ident_hash(text):
    """FNV-1a 32 of TEXT, with 0 remapped to 1.
       ...
       A hash rather than a table ordinal because an ordinal renumbers
       whenever a pattern is added to or removed from the .decode file,
       which would silently invalidate any table a consumer had keyed on
       it.  0 is reserved by the plugin API for 'no identity recorded'."""
    h = 0x811c9dc5
...
def output_ident_table():
    ...
    for p in ident_rows:
        qual = ident_qual_name(p)
        output('    { 0x', format(ident_hash(qual), '08x'), 'u, "', qual,
               '" },  /* ', os.path.basename(p.file), ':', str(p.lineno),
               ' */\n')
    output('};\n\n')
...
def output_autogen():
    ...
    if ident_export:
        output('#include "exec/plugin-gen.h"\n\n')
...
class Pattern(General):
    ...
    def output_code(self, i, extracted, outerbits, outermask):
        ...
        if ident_export:
            # Record on the branch where trans_ returned TRUE.  A pattern
            # that matches the bits and then declines (unimplemented
            # extension, wrong CPU feature) is not the decode QEMU made,
            # and naming it would publish a decision the emulator did not
            # take.
            #
            # Recorded HERE, at each dispatch site, rather than once at a
            # merged tail the sites goto.  The merged form was written
            # first and measured second: routing every site through one
            # label keeps the chosen row index live across the whole
            # decoder, and on a decoder the size of disas_a64 that costs
            # more than the call sites it saves -- +40.2 host instructions
            # per translated instruction merged against +32.6 per-site on
            # aarch64, +17.4 against +14.3 on riscv64, over a
            ...
            output(ind, 'if (', translate_prefix, '_', self.name,
                   ...
                   ident_table_name(), '[', idx, '].id, ',
                   ident_table_name(), '[', idx, '].name);'
                   ' return true; }\n')
        else:
            output(ind, 'if (', translate_prefix, '_', self.name,
...
def main():
    ...
    for o, a in opts:
        if o in ('-o', '--output'):
            ...
            # For a decoder that is a PREDICATE rather than an instruction
            # decoder -- aarch64's disas_sme_fa64 answers "is this encoding
            # legal in streaming mode", so its OK/FAIL patterns are not
            # instruction identities and must not be published as such.
            ident_export = False
        else:
    ...
    if ident_export and len(allpatterns) != 0:
        build_ident_index()
    ...
    emit_ident = ident_export and len(allpatterns) != 0
