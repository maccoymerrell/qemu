...
- Before doing a large mechanical edit anywhere, grep for "generated by" / "Auto-generated" first.
  `champsim_tracer_mnemonic_audit.py` survives as the referee's vocabulary source: it reaches Capstone
  through the Python bindings, offline, and the per-ISA tables it used to emit are gone with the walk
  that read them.
