                               ...
                               uintptr_t host_pc)
{
    ...
    } else if (tcg_slice_armed && !(tb_cflags(tb) & CF_NOIRQ)) {
        /*
 ...
 buffer_overflow:
    ...
    if (unlikely(!tb)) {
        ...
        if (cpu->plugin_spec_mode) {
             ...
             * nested inside (the walk runs synchronously from a plugin
             * callback of that TB), and post-flush translation would
