...
void qemu_ident_report(GString *report)
{
    ...
    qemu_ident_survivors(&sv);
    {
        ...
        g_string_append_printf(report,
            ...
            "REACHED -- FIXED, and this is the record of what it was.  x86 "
            "`hlt` decodes through "
            "`[0xF4] = X86_OP_ENTRY0(HLT, chk(cpl0) svm(HLT))`; QEMU knew "
            "exactly which row it picked and held its mnemonic.  At CPL != "
            "0 the chk(cpl0) arm branches to gp_fault, and that used to sit "
            "BEFORE plugin_gen_record_insn_identity() in decode-new.c.inc, "
            "so the identity was withheld by ORDERING, not by absence.  The "
            "publish now happens at the row-selection point -- immediately "
            "after `.gen` is found non-NULL and ahead of every fault arm -- "
            "so a row that exists always states itself.\n"
            "      MEASURED, two probes, both guests dying by fault: a "
            "3-insn probe ending in `hlt` read 3 here before the move and "
            "reads 0 after; a probe ending in a load from an unmapped "
            "address -- row selected, identity PUBLISHED, fault raised at "
            "execution -- read 0 in both.  So `faulting instructions do not "
            "publish' was refuted before the fix and the fix changed "
            "nothing about it; only a check sitting before the publish site "
            "ever withheld.\n"
            "      A wrong-path walk reaches bytes of kind (a), which is "
            "why this count need not be 0 on aarch64.  Cause (a) is a true "
            "floor -- no row exists, so nothing can be stated and the "
            "deletion cannot recover it.  Cause (b) is closed: it was a "
            "QEMU-side statement this fork could make, and it is made.\n"
            ...
            "    SPLIT is what a rule reports when several Capstone "
            "constants were observed decoding through it and the tables "
            "answer them differently.  On an ISA whose rules STATE their "
            "class it cannot arise: the class comes from the rule and the "
