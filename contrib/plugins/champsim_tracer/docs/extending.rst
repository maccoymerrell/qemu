...
3. **Classify the instruction.**  This is the only ISA-aware step.
   ...
   ``decode_detail_to_generic`` calls ``classify_insn_id()``, which
   asks ``qemu_ident_classify()`` for the row QEMU's own decode
   identity selects and writes the opcode into ``InsnFields.opcode``.
   The rows live in ``champsim_tracer_qemu_ident_<isa>.h`` and are
   **auto-generated**: each header carries a generation stamp naming
   the commit it was emitted against.  Do not hand-edit the rows.  A
   classification change goes through
   ``champsim_tracer_mnemonic_audit.py``'s regeneration path: teach
   the script's classification rules the new mnemonic, then re-emit
   with ``champsim_tracer_mnemonic_audit.py --isa <isa> --qemu-ident
   --apply``.  ``ident_header_gate.sh`` re-derives all four shipped
   headers and fails if any differs from the generator's output, so a
   hand-edit is caught rather than shipped.

   The Capstone-enumerator-keyed table this step used to index —
   ``x86_insn_class[]`` and its ``aarch64`` / ``mips`` / ``riscv``
   counterparts — is **retired under ruling R14**.  Its former home in
   ``champsim_tracer_mnemonics_<isa>.h`` carries a block saying so.
   Those headers still hold the register tables, ``<isa>_reg_class[]``,
   which ``--isa <isa> --regs --apply`` regenerates.  The bare
   ``--isa <isa> --diff`` / ``--apply`` that read and rewrote the
   instruction table now REFUSES, naming the retirement and the three
   modes that do have a subject; it does not run empty, because a
   generator that silently writes nothing is the failure this
   directory keeps meeting.

   Running ``champsim_tracer_mnemonic_audit.py --isa <isa>
   --qemu-ident --diff`` reports any decode identity missing a
   classification row, and prints the unclassified set.
