...
/*
 * QEMU's OWN decode identity, per ISA -- the decodetree pattern the
 * translator dispatched on, keyed by the id qemu_plugin_insn_decode_id()
 * reports.  x86_64 has no entry and that is not an omission: the i386 leg
 * derives its id from the source line of an X86_OP_ENTRY expansion, which
 * is not a decodetree pattern and has no generated universe to tabulate
 * (cst_runs/p3/arc3/DECODE_IDENTITY_CONTRACT.md).  A NULL table is the
 * honest statement of that, and the reader reports it as such rather than
 * counting the target's instructions as unidentified.
 */
