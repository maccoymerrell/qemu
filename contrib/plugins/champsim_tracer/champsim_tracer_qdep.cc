...
static const DstAdjRow g_dst_adj_ledger[] = {
    ...
    { TRACE_ISA_AARCH64, 0x822f9c49u, "dc", REG_SYSCACHE, SRC_ADJ_R16,
      ...
      "architecture's" },
    { TRACE_ISA_AARCH64, 0x5c29c765u, "smstop", REG_VCTRL, SRC_ADJ_OWED,
      "QUESTION ON FILE, and it BLOCKS.  `smstop` is `msr SVCR<sm|za>, #0` "
      "and it WRITES SVCR: QEMU performs it, in helper_set_svcr(), into "
      "env->svcr, and the boundary publishes it as REG_VCTRL from the "
      "AARCH64_OP_SVCR sysop operand with READ|WRITE access.  The write "
      "therefore happens on both sides and QEMU still cannot state it: "
      "env->svcr has NO TCG global, NO regfile declaration, and -- the "
      "part that makes this a question rather than a missing note -- NO "
      "NAME IN QEMU'S GDB-STUB NAMESPACE, which is the namespace "
      "champsim_tracer_qemu_regs_aarch64.h is generated from.  A stated "
      "range would arrive downstream anonymous, and hand-adding a "
      "vocabulary row to a generated table is forbidden.  THE QUESTION: "
      "does SVCR get a QEMU gdbstub register (the upstream fix, which "
      "would make the table generate the row and the note resolve), or a "
      "generic id of its own instead of borrowing REG_VCTRL, which today "
      "is the SVE `vg` granule?  Until one is answered the row blocks; "
      "nothing on the wire is decided by it" },
};
