                                   ...
                                   uint32_t vp_index, uint64_t now)
{
    ...
    /*
     * A target that is not in the future (compare equals the count, or is
     * behind it) would next match a full 2^32 ticks away, 42.95 s at this
     * timer's 100 MHz.  Re-arm at 2^24 ticks (167.8 ms) instead, the bound
     * cpu_mips_timer_update() in target/mips/system/cp0_timer.c uses for the
     * same arithmetic.  The equality case must not arm at @now:
     * timerlist_run_timers() counts a deadline equal to its once-sampled
     * current time as expired, so on a stopped virtual clock this callback
     * would be re-run forever with the BQL held.
     */
