...
def main():
    ...
    if refused:
        ...
        print('')
        print('AND THE NUMBER STILL HAS TO BE CAUSED.  Refusing the root ends')
        print('its career as a fixture and answers nothing about why it reads')
        print('what it reads; a root retired without a cause is a red nobody')
        print('is looking at any more.  The worked example is verify55, whose')
        print('gem5wp/aarch64 30 was CAUSED at PASS 74: it is a real red the')
        print('same wave found and repaired, and it reproduces nowhere for the')
        print('most ordinary reason there is -- the comparator that produced')
        print("it does not exist at any later tip.  That root's OWN later")
        print('roots, r13b and r13c, already read 0.  Look for that shape')
        print('first: an opening measurement mistaken for a wave result.')
        if absent_rows:
            print('')
