 ...
 *
 * Every interaction with the sink is a plain blocking write.  A
 * compressor that reads slowly is waited for — what it is finishing is
 * the trace — and a command that never reads its stdin stops the
 * tracer with it, which is the honest rendering of an operator error:
 * the bytes have nowhere to go.  (A full output FILESYSTEM is a
 * different failure and does not block: the compressor's own write
 * fails, it exits, and the writer takes a short write on the broken
 * pipe, reported below as "writer fwrite short".)
 *
