 ...
 * thing QEMU ran.
 *
 * .tier is the PROVENANCE of the row's classification and the three are
 * not interchangeable.  QID_OBSERVED means QEMU was seen decoding through
 * the rule and the disassembler named the result; QID_NAME_MATCHED means
 * the pattern name matches a Capstone mnemonic and nothing was ever
 * observed decoding through it; QID_NONE means neither.  Reading the
 * table is what turns a tier into a measurement -- a QID_NAME_MATCHED row
 * that the audit then observes is a row the table UNDERSTATES, and a
 * QID_NONE row that executes is live residue.
 *
