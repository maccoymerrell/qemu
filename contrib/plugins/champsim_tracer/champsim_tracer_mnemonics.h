...
/*
 * QEMU's own DECODE identity, per ISA.
 *
 * A row is one decodetree pattern -- the trans_<name>() the translator
 * dispatched to, i.e. the decision the emulator actually made -- keyed by
 * the id qemu_plugin_insn_decode_id() reports for an instruction decoded
 * through it.  Where the register tables answer "which register", these
 * answer "which rule", and for the same reason: the tracer's identity has
 * otherwise always come from Capstone, a disassembler that is not the
 * thing QEMU ran.
 *
 * .tier is the PROVENANCE of the row's classification, and the tiers fall
 * into two eras that are not interchangeable.
 *
 * On a STATED ISA the class is a static fact of QEMU's decode rule, read
 * from the rule's own words at generation time, and coverage is closed by
 * construction -- a rule that states nothing is a build error, not a row.
 * Those tables carry only QID_STATED and QID_VERIFIED, and the difference
 * between them is whether anything INDEPENDENT has yet agreed with the
 * statement; neither is a fallback and neither is missing an answer.
 *
 * The older tiers say where an OBSERVATION-sourced payload came from, and
 * survive on the ISAs whose vocabulary has not been taught yet.
 * QID_OBSERVED means QEMU was seen decoding through the rule and the
 * disassembler named the result; QID_NAME_MATCHED means the pattern name
 * matches a Capstone mnemonic and nothing was ever observed decoding
 * through it; QID_NONE means neither.  Reading the table is what turns
 * one of those into a measurement -- a QID_NAME_MATCHED row that the
 * audit then observes is a row the table UNDERSTATES, and a QID_NONE row
 * that executes is live residue.
 *
 * Rows are auto-generated by champsim_tracer_mnemonic_audit.py
 * --qemu-ident and sorted by id, so a consumer bisects.  The type lives
 * here rather than in each generated header because a consumer routing by
 * TraceISA includes all of them at once.
 */
enum QemuIdentTier {
    QID_NONE = 0,
    QID_NAME_MATCHED,
    QID_OBSERVED,
    /*
     * Observed, and the observations DISAGREED: several classification
     * rows were seen decoding through this one rule.  The row carries
     * GEN_OP_UNKNOWN deliberately -- picking the most frequent made the
     * generated file depend on which programs happened to run, and
     * picking at all would be averaging two behaviours into a third
     * that describes neither.  Resolving one of these needs the
    ...
    QID_ADJUDICATED,
    /*
     * QEMU's own decode RULE states the class, and nothing independent
     * has been read against it.
     *
     * This is not a weaker QID_OBSERVED.  The class of a rule is a
     * compile-time property of the decoder -- the generator reads the
     * rule's own pattern name, the format it references where that
     * format pins an architectural field, and the fields the rule's line
     * pins -- so there is nothing an execution could add to it.  What a
     * corpus can still do is DISAGREE, and that is what the tier above
     * records.
     */
    QID_STATED,
    /*
     * Stated, and an independent reading AGREED: QEMU was seen decoding
     * through the rule and a disassembler, which is not the decoder,
     * named the same class.
     *
     * This is what QID_OBSERVED became.  The observation is no longer
     * where the classification comes from; it is the check that passed,
     * and a row carries this tier to say the check ran and held.  A
     * disagreement never reaches a table: the generator refuses to
     * emit one unless it is ruled, with the QEMU source fact that
     * settles it written beside the row.
     */
    QID_VERIFIED,
    /*
     * Not a tier.  The count, so an array indexed by tier is sized by
     * the enum and cannot silently fold an unknown tier into a real one
     * -- which is exactly what a hand-written [4] once did to every
     * QID_ADJUDICATED row.
     */
    QID_TIER_COUNT,
};
...
struct QemuIdentRow {
    ...
    bool     cap_split;
    /*
     * The SAME payload the Capstone-keyed table carries, from the same
     * classifier, under this key instead.  It is a whole
     * InsnClassification and not an opcode/branch pair because the
     * classification is not two fields: .refine, .dep_refine and the
     * lane pair are as much a part of what an instruction IS, and a row
     * carrying half of it would be a second, quieter answer.
     */
    InsnClassification cls;
};
...
#include "champsim_tracer_qemu_ident_x86.h"
#include "champsim_tracer_qemu_ident_aarch64.h"
#include "champsim_tracer_qemu_ident_riscv.h"
#include "champsim_tracer_qemu_ident_mips.h"
...
/*
 * QEMU's OWN decode identity, per ISA -- the rule the translator
 * dispatched on, keyed by the id qemu_plugin_insn_decode_id() reports.
 *
 * ALL FOUR TARGETS HAVE A TABLE.  x86_64 used to have none, on the
 * reasoning that its id is the source line of an X86_OP_ENTRY expansion
 * rather than a decodetree pattern and therefore had "no generated
 * universe to tabulate".  That was a statement about where the generator
 * looked, not about the machine: the universe is the decode table
 * itself, `target/i386/tcg/decode-new.c.inc`, and the same scan that
 * produces the id produces it.  Reading the source table also recovers
 * the one fact the generated tree does not carry -- the X86_OP_* macro
 * the row was written with, which is what says a row named SUB computes
 * its result and discards it, i.e. is `cmp`.
 */
extern const QemuIdentRow *const isa_qemu_ident[];
const QemuIdentRow *const isa_qemu_ident[] = {
    [TRACE_ISA_UNKNOWN] = NULL,
    [TRACE_ISA_X86]     = qemu_ident_x86,
    [TRACE_ISA_AARCH64] = qemu_ident_aarch64,
    [TRACE_ISA_RISCV]   = qemu_ident_riscv,
    [TRACE_ISA_MIPS]    = qemu_ident_mips,
};

extern const unsigned isa_qemu_ident_count[];
const unsigned isa_qemu_ident_count[] = {
    [TRACE_ISA_UNKNOWN] = 0,
    [TRACE_ISA_X86]     = (unsigned)qemu_ident_x86_count,
    [TRACE_ISA_AARCH64] = (unsigned)qemu_ident_aarch64_count,
    [TRACE_ISA_RISCV]   = (unsigned)qemu_ident_riscv_count,
