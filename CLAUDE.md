...
../configure --enable-plugins --disable-capstone \
...
The repository carries **no Capstone**: no wrap, no vendored copy, no generated tables, and nothing it
builds links the library. `--disable-capstone` is part of the documented flow so a host that happens to
have a system Capstone cannot pull one into QEMU's own disassembler either; `disas/capstone.c` and the
meson feature option are upstream's, untouched. The reference decoder the acceptance comparison scores
...
- **Plugin is C++17 throughout** (one TU per subsystem in `contrib/plugins/champsim_tracer/`). The only C
  boundary is the QEMU plugin entry points. Do **not** add C TUs or `extern "C"` wrappers.
...
- Before doing a large mechanical edit anywhere, grep for "generated by" / "Auto-generated" first.
  `champsim_tracer_mnemonic_audit.py` survives as the referee's vocabulary source: `cst_referee.py`
  imports its rule functions (`ISAS`/`classify`/`classify_reg`), and it is the REFEREE that reaches
  Capstone — offline, through the Python bindings. The audit's own `--diff`/`--apply` modes are
  retired and refuse loudly: the vendored headers they parsed and the per-ISA tables they rewrote
  are gone with the walk that read them.
- The 4-per-slot dynamic **lane-mask model** (src / dst / load / store) is the required design: the
  ...
  memop layout.
- When the root cause of a bug is **upstream** (QEMU, glibc), report it as a bug with a fix path —
  don't automatically downgrade it into a documented plugin limitation. Capstone defects are now the
  offline referee's business alone: they cannot reach the wire, and the workarounds that used to live
  at the boundary are ported into `tools/cst_referee.py`.
...
| `contrib/plugins/champsim_tracer/tools/cst_referee.py` | The offline referee — Capstone's column of the two-decoder comparison, through the Python bindings |
