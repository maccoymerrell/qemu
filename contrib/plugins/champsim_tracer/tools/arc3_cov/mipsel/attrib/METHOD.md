...
| --- | ---: | --- |
| `L-AT` | 27 | LLVM defect. Every MIPS branch carries `Defs=[AT]`, an artifact of the assembler's long-branch expansion, not an architectural write (**R2**). binutils names no such write (`beq`'s pinfo is `COND_DELAY_SLOT RD_op1 RD_op2`). Removed from the reference -- **the tracer is right**. The rule is keyed on the OBSERVED LLVM record: an `impDEF AT` on a branch whose disassembly names no `$1`. It was once a list of four mnemonics, and the other 23 rows of the class went unhandled because the tracer carried the same phantom `$at` write and the two sides agreed on a fact false on both; removing the tracer's phantom (95a0d89e92) surfaced them as 23 `TRACER-SUBSET` rows. |
