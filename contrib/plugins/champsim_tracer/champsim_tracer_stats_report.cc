                                 ...
                                 const Stats &stats)
{
    const struct { const char *name; uint64_t value; } counters[] = {
        ...
        { "entries emitted with an UNRESOLVED terminal branch",
                                                 stats.branch_outcome_unresolved_cp },
        { "  wrong-path blocks likewise",        stats.branch_outcome_unresolved_wp },
        ...
        { "census frames unwound-DROPPED by the anchor guard (must be 0)",
                                                stats.census_frames_unwound_guard_dropped },
        { "  insns that guard drop discarded",
                                                stats.census_frames_unwound_guard_insns },
        ...
        { "holders a close DISCARDED without emitting (must be 0)",
                                                stats.close_holder_discarded },
        { "  insns that discard threw away",
                                                stats.close_holder_discarded_insns },
