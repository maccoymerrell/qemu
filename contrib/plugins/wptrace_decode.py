...
def reg_name(reg_id: int) -> str:
    ...
    if 33 <= reg_id <= 64:
        return f"FPR{reg_id - 33}"
    if 65 <= reg_id <= 96:
        return f"VEC{reg_id - 65}"
    if reg_id == 250:
        return "SP"
    if reg_id == 251:
        return "FLAGS"
...
def decode_wptrace(bin_path: Path) -> tuple[list[dict], list[dict]]:
    ...
    if magic == WPT_MAGIC_V9:
        ...
        raise ValueError(
            ...
            f"or 0x{WPT_MAGIC_V10:08x}"
        )

    _isa = br.read_bits(WPT_ISA_BITS)
...
def render_text(templates: list[dict], entries: list[dict]) -> str:
    out: list[str] = []
    ...
    for tmpl in templates:
        ...
        for i, insn in enumerate(tmpl["insns"]):
            ...
            src = ",".join(reg_name(r) for r in insn["src_regs"])
            dst = ",".join(reg_name(r) for r in insn["dst_regs"])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Decode wptrace binary (.bin, v9/v10) and reconstruct debug text format"
        )
    )
    ...
    templates, entries = decode_wptrace(args.bin)
    text = render_text(templates, entries)
