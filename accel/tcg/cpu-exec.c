 ...
 */
inline TranslationBlock *tb_lookup(CPUState *cpu, vaddr pc,
                                   uint64_t cs_base, uint32_t flags,
                                   uint32_t cflags)
{
    TranslationBlock *tb;
 ...
 */
inline TranslationBlock * QEMU_DISABLE_CFI
 ...
 * with it and the next translation touching that page spins forever; in user
 * mode a leaked pointer is silent, but it still names a translation that is
 * no longer in flight.
 *
...
static void cpu_exec_longjmp_cleanup(CPUState *cpu)
{
     ...
     *
     * This is the ABNORMAL counterpart of qemu_plugin_spec_mode_end() +
     * qemu_plugin_cpu_state_restore() + qemu_plugin_spec_vtime_resume():
     * when a speculative exception/longjmp escapes the plugin's wrong-path
     * run it lands here instead, so it must undo everything the excursion
     * set up -- including resuming the guest virtual clock paused for it.
     * Omitting the vtime resume leaves the clock frozen: QEMU_CLOCK_VIRTUAL
     * (hence the guest's CNTVCT/TSC/time/Count) stops for good, and the
     * guest's timer subsystem livelocks.  The close is idempotent
     * (plugin_excursion_active), so this is safe even if the plugin's normal
     * exit later runs too.
     */
    if (cpu->plugin_spec_mode || cpu->plugin_excursion_active) {
        ...
        cpu_plugin_excursion_close(cpu);
    }
     ...
     * depth stuck above zero, cpu_disable_ticks()
                                    ...
                                    int *tb_exit)
{
    ...
    /*
     * Diagnostic (CST_SPEC_ESCAPE_DIAG): the main loop only runs REAL TBs;
     * speculative wrong-path TBs run via cpu_plugin_exec_tb/_inline.  If
     * plugin_spec_mode is set here, a WP excursion leaked spec mode into real
     * execution (would make riscv stimecmp writes bail -> timer death) -- a
     * spec-mode escape.
     */
    ...
    *last_tb = NULL;
    ...
    /* Guest-insn slice expired (default clock).  */
    if (unlikely(tcg_slice_armed) && !icount_enabled()) {
        /*
         ...
         * >= tb->icount, asserted.
         */
        tcg_slice_note_breakout(cpu->neg.icount_decr.u16.low);
        g_assert(tb->icount <= tcg_slice_quantum);
#if !defined(CONFIG_USER_ONLY)
        /*
         * Event-agency consumption (PRODUCT; see qemu/vclock-agency.h):
         * THE product consumption site -- every consumption happens at
         * a guest-insn slice breakout.  The predicate is a FRESH
         * qemu_clock_deadline_ns_all(VIRTUAL, ATTR_ALL) == 0 read
         * (icount_handle_deadline's own test), NOT a cached compare.
         * The spec-mode skip lives inside vclock_agency_consume.  Delivery lag is bounded in
 ...
 * cpu_plugin_excursion_open/_close) does not exist.  The product
 ...
 * edge ever changes.
 */
static void tcg_slice_posture_check(void)
{
    static bool tcg_slice_checked;

    if (tcg_slice_checked) {
        return;
    }
    tcg_slice_checked = true;
#ifdef CONFIG_USER_ONLY
...
cpu_exec_loop(CPUState *cpu, SyncClocks *sc)
{
     ...
     */
    if (unlikely(qatomic_read(&vclock_agency_active)) && !tcg_slice_armed) {
    ...
    while (!cpu_handle_exception(cpu, &ret)) {
        ...
        while (!cpu_handle_interrupt(cpu, &last_tb)) {
             ...
             * register a plugin derives guest-thread identity from.  Under
             * SMP a PEER thread scheduled onto this vCPU inside the window
             * can execute the same VA, and a bare PC equality would close
            ...
            if (unlikely(cpu->plugin_in_async_int) &&
                 ...
                 * VA passes the (pc, tp) test and would close the departed
                 * process's window, leaving
