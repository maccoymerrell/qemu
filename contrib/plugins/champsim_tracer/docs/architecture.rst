...
.. list-table::
   ...
   * - ``champsim_tracer_decode.cc``
     - Turns one decoded instruction into ``GenericOpcode`` /
       ``BranchType`` / ``GenericRegId``.  The opcode, branch type and
       flag word come from QEMU's own decode identity, through
       ``qemu_ident_classify()`` in ``champsim_tracer_qemu_ident.cc``;
       the register vocabulary is this file's.
   ...
   * - ``champsim_tracer_mnemonics_<isa>.h``
     ...
     - The per-ISA **register** tables: ``<isa>_reg_class[]``, which
       maps a Capstone register ID to a ``GenericRegId`` plus its
       GDB-stub feature/name.  Auto-generated by
       ``champsim_tracer_mnemonic_audit.py --regs``.  The instruction
       table ``<isa>_insn_class[]`` that used to sit beside it is
       **retired under ruling R14** — each header carries a block at
       the place it stood saying so.  Instruction classification is
       ``champsim_tracer_qemu_ident_<isa>.h``'s, keyed on QEMU's decode
       identity rather than on a Capstone enumerator.
   * - ``champsim_tracer_mnemonic_tables.cc``
     - The shared dependency-refiner functions the classification
       tables' ``.dep_refine`` slots point at.  Five survive --
       ``dep_passthrough``, ``dep_lea``, ``dep_x86_stack_push``,
       ``dep_x86_stack_pop`` and ``dep_vec_struct_store``.  A row that
       states nothing beyond the reader's own default carries no
       refiner at all: ``format.rst`` defines the absence of a
       dependency block as the all-to-all over-approximation, so a
       refiner that published it was spending bytes to say what the
