...
static const DstAdjRow g_dst_adj_ledger[] = {
    ...
    { TRACE_ISA_X86,   0xdb9bac2bu, "rdsspq", REG_GPR0, SRC_ADJ_R16,
      "ADJUDICATED-KEEP-R16, the write twin of this instruction's REG_SSP "
      "source row two tables up.  RDSSPQ's whole effect is to put the "
      "shadow-stack pointer INTO the GPR its ModRM names, and both "
      "reference decoders that model x86 operand access say so on the same "
      "bytes: XED, decoding f3480f1ec8 with CET enabled, reports "
      "`REG0 EXPLICIT W R=0 W=1 reg=RAX` beside the suppressed SSP read, "
      "and iced-x86 reports `RAX access=WRITE`.  QEMU models no CET, so "
      "F3 0F 1E /1 lands in decode-new.c.inc's NOP slot and writes nothing "
      "-- a statement gap, not an architectural fact, and the same gap the "
      "source row already carries in the other direction.  R16 records the "
      "dependency regardless of the modelled machine's CET state" },
    { TRACE_ISA_X86,   0x000004b1u, "syscall", REG_FLAGS, SRC_ADJ_R16,
      "ADJUDICATED-KEEP-R16.  Long-mode SYSCALL writes RFLAGS -- "
      "`RFLAGS <- RFLAGS AND NOT(IA32_FMASK)`, with RF cleared (Intel SDM "
      "vol.2 SYSCALL, AMD APM vol.3), and XED reports it on 0f05 as "
      "`REG3 SUPPRESSED W R=0 W=1 reg=RFLAGS` with must_write set.  The "
      "instruction's other two architectural destinations, RCX and R11, "
      "ARE now stated at gen_SYSCALL(); this register is not, and the "
      "reason differs by mode, which is why one ruling covers both rather "
      "than two rows disagreeing about one instruction.  In *-linux-user "
      "the masking is DELIBERATELY not emulated: IA32_FMASK belongs to a "
      "kernel this mode does not have and the SYSRET the real kernel "
      "returns through undoes it, so a process must observe RFLAGS "
      "unchanged across a syscall -- which is what it observes natively "
      "(target/i386/tcg/user/seg_helper.c carries the measurement).  In "
      "softmmu the write HAPPENS, inside helper_syscall(), through "
      "env->eflags rather than the lazy cc_* quartet; no op names those "
      "bytes and no regfile declares them, so QEMU's ordered write list is "
      "silent there too.  Either way the register is on the wire because "
      "the ISA puts it there, and a destination-list flip must carry it.  "
      "The row is superseded, in softmmu only, by declaring env->eflags "
      "and stating the write at the emitter -- a change whose own R13 "
      "execution legs need a system guest, and which is not landed here" },
    { TRACE_ISA_AARCH64, 0x822f9c49u, "dc", REG_SYSCACHE, SRC_ADJ_R16,
      "ADJUDICATED-KEEP-R16.  `dc <op>, Xt` is a data-cache maintenance "
      "operation and the CACHE is its destination: the ARM ARM defines DC "
      "ZVA as writing a naturally-aligned block and the CVAC/CIVAC/CVAU "
      "family as changing the cache's own tag and data state.  The wire "
      "carries that as REG_SYSCACHE, added at the boundary by "
      "cap_aarch64_add_sysreg() from the sysop encoding itself -- LLVM MC "
      "reports `WR{-}` for the whole SYS opcode space, so the boundary is "
      "the only decoder in the set that can express the operation's "
      "target at all, and that is a deliberate contract and not a repair.  "
      "QEMU MODELS NO CACHE: gen_helper_dc_zva() writes guest MEMORY and "
      "every other DC form is a no-op, so there is no CPUARMState member "
      "for the write list to name.  R16 records the dependency because it "
      "exists in the ISA; the modelled machine's cache is not the "
      "architecture's" },
    { TRACE_ISA_AARCH64, 0x5c29c765u, "smstop", REG_VCTRL, SRC_ADJ_R16,
      "RULED, and the question that blocked it is answered by a THIRD "
      "route neither of its two arms named.  `smstop` is "
      "`msr SVCR<sm|za>, #0` and it WRITES SVCR: QEMU performs the write "
      "in helper_set_svcr() into env->svcr, and the boundary publishes it "
      "as REG_VCTRL from the AARCH64_OP_SVCR sysop operand.  What was "
      "missing was never the write and never the vocabulary -- it was the "
      "NAME: env->svcr had no TCG global and no regfile declaration, so "
      "QEMU's statement arrived downstream as an anonymous byte range.  "
      "The question asked whether SVCR needed a new gdbstub register or a "
      "generic id of its own.  Neither.  It needed the declaration every "
      "register of its shape already has -- "
      "insn_dataflow_declare_regfile(\"svcr\", ...) in "
      "target/arm/tcg/translate-a64.c, beside tpidr_el0, cpacr_el1 and "
      "gcr_el1, three registers that are likewise real env accesses with "
      "no TCG global and no GDB-stub row -- and the fold those three and "
      "riscv's `elp` and x86's `xcr0` all take.  Nothing is hand-added to "
      "a generated table.  AND REG_VCTRL IS THE ROLE, NOT THE RESIDUE: "
      "REG_VCTRL is the word for the state that says how wide a vector "
      "operation is and how much of it is active -- SVE's `vg` and "
      "RISC-V's `vl`/`vtype` seat there -- and SVCR.SM selects between "
      "the non-streaming and the streaming vector length while SVCR.ZA "
      "gates the ZA array.  A generic id of its own would split one "
      "dependence class in two and make an SME instruction's read of the "
      "effective vector length a different register from an SVE "
