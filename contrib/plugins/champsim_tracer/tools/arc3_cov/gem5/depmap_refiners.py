...
def table_families(qemu_dir, isa, rev=RETIREMENT + '^', allow_absent=False):
    ...
    were read; the default reads the retirement's parent, which is the only
    place the four retired families still exist.

    THE WORKING-TREE READ HAS NO SUBJECT SINCE THE CAPSTONE DELETION, and
    that is a FACT about the tree rather than an error in this file.
    ``c32824defa`` removed ``champsim_tracer_mnemonics_<isa>.h`` outright --
    the classification tables were themselves generated by a Python script
    driving the Capstone bindings, and the maintainer's ruling is that the
    repository carries no Capstone -- so at HEAD there is no table to read
    and NO refiner family survives in the shipped plugin: ``.dep_refine``
    appears only in comments.  ``allow_absent=True`` returns an empty map so
    the caller can SAY SO on its own report; without it the read raises,
    because a silent ``{}`` would make every at-HEAD family lookup miss and
    print as though the encodings carried no family, which is the
    silent-false-success shape this tree files against.
    """
    fname, _prefix = _TABLES[isa]
    path = 'contrib/plugins/champsim_tracer/' + fname
    if rev is None:
        full = '%s/%s' % (qemu_dir, path)
        if not os.path.exists(full):
            if allow_absent:
                return {}
            raise SystemExit(
                'depmap_refiners: REFUSING -- %s does not exist.\n'
                '  The generated classification tables were deleted at\n'
                '  c32824defa, so "the family this row carries NOW" has no\n'
                '  source in this tree and the answer is EMPTY rather than\n'
                '  unknown.  Pass allow_absent=True to receive that empty\n'
                '  map and report it; an unguarded read must not turn a\n'
                '  deleted table into a silent set of misses.' % full)
...
class Attributor(object):
    ...
    def __init__(self, qemu_dir, isa, rev=RETIREMENT + '^'):
        ...
        # The at-HEAD column: EMPTY since the table deletion, and the
        # emptiness is recorded so the report can print it as a stated
        # fact instead of as a column of misses.
        self.now = table_families(qemu_dir, isa, rev=None, allow_absent=True)
        self.now_absent = not self.now
    ...
    def family(self, pc, bits):
        ...
        if ins is not None:
            ...
            if enum is None:
                ...
                # Named apart when the at-HEAD half has no table at all:
                # 'the row is not in either table' and 'one of the two
                # tables no longer exists' are different facts and a
                # shared counter would let the second read as the first.
                self.miss['enum-not-in-table' if not self.now_absent
                          else 'enum-not-in-retired-table'
                          ' (the at-HEAD table was deleted)'] += 1
                res = (None, None, enum)
            else:
