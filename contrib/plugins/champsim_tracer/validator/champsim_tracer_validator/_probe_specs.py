...
_register_probe('probe_rv_v0_carry_mask', {
    'riscv64': {
        ...
        #
        # REG_VCTRL IS A DESTINATION OF EVERY VECTOR INSTRUCTION HERE, not
        # just of the vsetvli.  RVV v1.0 §3.7 makes `vstart' architectural
        # state that an instruction RESETS TO ZERO when it retires without a
        # trap, and vstart is one of the fields this container folds; QEMU
        # performs that reset in finalize_rvv_inst() / finalize_vset_inst()
        # (target/riscv/translate.c), which is why the wire names it.  It is
        # written by a helper rather than by a TCG store to the global, so no
        # op in the stream carries it and target/riscv/translate.c states it
        # at the decode site instead.  The earlier declaration -- vsetvli
        # alone writing REG_VCTRL -- described the vtype/vl half of the
        # container and forgot the vstart half.
        'reg_sets': [
            # vsetvli t0, zero, e64, m1, ta, ma
            #
