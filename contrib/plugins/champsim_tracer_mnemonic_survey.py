#!/usr/bin/env python3
"""
champsim_tracer_mnemonic_survey.py — Static mnemonic coverage analyzer.

Parses champsim_tracer_mnemonics.h to build the set of known instruction IDs
...
import capstone.aarch64_const as aarch64_const
...
HEADER_REL = os.path.join(os.path.dirname(__file__), "champsim_tracer_mnemonics.h")

# ISA → (table C identifier, Capstone const module, constant prefix)
ISA_TABLE_INFO = {
    "x86":     ("x86_insn_class",     x86_const,     "X86_INS_"),
    "aarch64": ("aarch64_insn_class", aarch64_const, "AARCH64_INS_"),
...
def _arch_member(*names):
    for name in names:
        if hasattr(lief.ELF.ARCH, name):
            return getattr(lief.ELF.ARCH, name)
    raise AttributeError(f"lief.ELF.ARCH missing expected names: {names}")


# lief.ELF.ARCH → (capstone_arch, capstone_mode, isa_key)
ARCH_MAP = {
    _arch_member("x86_64", "X86_64"): (capstone.CS_ARCH_X86, capstone.CS_MODE_64, "x86"),
    _arch_member("i386", "I386"): (capstone.CS_ARCH_X86, capstone.CS_MODE_32, "x86"),
    _arch_member("AARCH64", "aarch64"): (capstone.CS_ARCH_AARCH64, capstone.CS_MODE_ARM, "aarch64"),
    # RISC-V: standard ratified extensions only. Vendor extensions (XTHEAD*,
    # XSF*, XCV*, SIFIVE, COREV) and ZFINX/ZHINX/E/ZCMP conflict with FD/RVC.
    _arch_member("RISCV", "riscv"): (capstone.CS_ARCH_RISCV,
                            ...
                            "riscv"),
    # MIPS: default to 32-bit; bit-width refined below from ELF class.
    _arch_member("MIPS", "mips"): (capstone.CS_ARCH_MIPS, capstone.CS_MODE_MIPS32, "mips"),
}
...
def make_disassembler(binary):
    ...
    # LIEF API compatibility: data endianness enums moved from
    # ELF.ELF_DATA to ELF.Header.ELF_DATA in newer releases.
    elf_data_msb = None
    if hasattr(lief.ELF, "ELF_DATA"):
        elf_data_msb = lief.ELF.ELF_DATA.MSB
    elif hasattr(lief.ELF, "Header") and hasattr(lief.ELF.Header, "ELF_DATA"):
        elf_data_msb = lief.ELF.Header.ELF_DATA.MSB

    # For MIPS: add big/little endian
    if isa == "mips":
        if elf_data_msb is not None and binary.header.identity_data == elf_data_msb:
            cs_mode |= capstone.CS_MODE_BIG_ENDIAN
        else:
            cs_mode |= capstone.CS_MODE_LITTLE_ENDIAN

    # LIEF API compatibility: ELF class enums moved from ELF.ELF_CLASS
    # to ELF.Header.CLASS in newer releases.
    elf_class32 = None
    elf_class64 = None
    if hasattr(lief.ELF, "ELF_CLASS"):
        elf_class32 = lief.ELF.ELF_CLASS.CLASS32
        elf_class64 = lief.ELF.ELF_CLASS.CLASS64
    elif hasattr(lief.ELF, "Header") and hasattr(lief.ELF.Header, "CLASS"):
        elf_class32 = lief.ELF.Header.CLASS.ELF32
        elf_class64 = lief.ELF.Header.CLASS.ELF64

    # For RISC-V: detect 32 vs 64 from ELF class (keep all extension bits).
    if isa == "riscv":
        if elf_class32 is not None and binary.header.identity_class == elf_class32:
            cs_mode = (cs_mode & ~capstone.CS_MODE_RISCV64) | capstone.CS_MODE_RISCV32

    # For MIPS: switch to 64-bit when ELF class is CLASS64.
    if isa == "mips":
        if elf_class64 is not None and binary.header.identity_class == elf_class64:
            # Replace the 32-bit width bit with 64-bit, keep endianness.
            cs_mode = (cs_mode & ~capstone.CS_MODE_MIPS32) | capstone.CS_MODE_MIPS64
...
def collect_insn_ids(binary, md):
    ...
    if hasattr(lief.ELF, "SECTION_FLAGS"):
        exec_flag = lief.ELF.SECTION_FLAGS.EXECINSTR
    elif hasattr(lief.ELF, "Section") and hasattr(lief.ELF.Section, "FLAGS"):
        exec_flag = lief.ELF.Section.FLAGS.EXECINSTR

    for section in binary.sections:
        if exec_flag is not None and not (section.flags & exec_flag):
            continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Survey ELF binaries for instruction IDs missing from champsim_tracer tables."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Path to champsim_tracer_mnemonics.h (default: auto-detect)",
    )
    parser.add_argument(
